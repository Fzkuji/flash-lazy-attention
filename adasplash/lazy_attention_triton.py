...
class LazyAttentionTritonFunc(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, bias, tau, window_size):
        out, lse = _lazy_attention_forward_return_lse(q, k, v, bias, tau, window_size)
        ctx.save_for_backward(q, k, v, bias, tau, lse)
        ctx.window_size = window_size
        return out
        
    @staticmethod
    def backward(ctx, do):
        q, k, v, bias, tau, lse = ctx.saved_tensors
        ...
        _lazy_attention_backward(do, q, k, v, bias, tau, lse, dq, dk, dv, dbias, dtau, window_size)
        
        return dq, dk, dv, dbias, dtau, None

def _lazy_attention_forward_return_lse(q, k, v, bias, tau, window_size):
    B, H, L, D = q.shape
    BLOCK_M = 64
    BLOCK_N = 64
...
def _lazy_attention_backward(do, q, k, v, bias, tau, lse, dq, dk, dv, dbias, dtau, window_size):
    B, H, L, D = q.shape
    BLOCK_M = 64
    BLOCK_N = 64
